...
export async function POST(request: Request) {
  ...
  if (countError) {
    return NextResponse.json(
      { error: "Failed to check feed count" },
      { status: 500 }
    );
  }
  ...
  if (currentFeedCount && currentFeedCount >= maxFeeds) {
    return NextResponse.json(
      {
        error: `フィードの追加上限に達しています。最大${maxFeeds}件まで追加できます。個別に連絡いただければ追加も可能です。`,
      },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    if (newItems.length > 0) {
      try {
        ...
        console.error(
          "Upsert failed, falling back to individual inserts:",
          error
        );
        // Fallback to individual inserts
        for (const item of newItems) {
    ...
    if (finalItems && finalItems.length > 50) {
      ...
      if (deleteError) {
        ...
        console.log(
          `Deleted ${itemsToDelete.length} excess items to maintain 50 item limit`
        );
      }
    }
